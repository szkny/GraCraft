...
inline void GRAPPA::Resize(int wx, int wy){
...
inline void GRAPPA::DrawDisplay(){
	if(STFLAG){
		...
		glColor3d(1,1,1);
		if(GFLAG){
			glDrawString("Mode : GRAPPA",15,15);
			char s[128];
			...
			glDrawString("Mode : Object",15,15);
		}
...
inline bool GRAPPA::GetGFLAG(){
	return GFLAG;
}
inline void GRAPPA::SetGFLAG(){
	if(GFLAG) GFLAG = false;
	else GFLAG = true;
}

inline void GRAPPA::Draw(){
